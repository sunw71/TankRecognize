...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 23 21:59:32 2019
...
# use vgg mean 
mp = MeanPreprocessor(123.68, 116.779, 103.939)
iap = ImageToArrayPreprocessor()

# initialize the training and validation dataset generators
trainGen = HDF5DatasetGenerator(config.TRAIN_HDF5, 32, aug=aug,
                                preprocessors=[mp, iap], classes=205)
...
#headModel = FCHeadNet.build(base_model, 205, 1024)
#
## place the head FC model on top of the base model -- this will
## become the actual model we will train
#model = Model(inputs=base_model.input, outputs=headModel)
#plot_model(model, to_file="FineTune_tank.png", show_shapes=True)
...
#                    validation_data=valGen.generator(), epochs=75,
#                    steps_per_epoch=trainGen.numImages // 128, verbose=1)
# train the network
model.fit_generator(
        trainGen.generator(),
